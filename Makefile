...
lint: ## check style with flake8 and pylint
	flake8 zoom_api_helper tests
	pylint zoom_api_helper tests
...
coverage: ## check code coverage with unit tests quickly with the default Python
	coverage run --source zoom_api_helper -m pytest tests/unit
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/zoom_api_helper.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ zoom_api_helper
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
